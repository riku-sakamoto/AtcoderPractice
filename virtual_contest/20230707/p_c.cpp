...
int main(){
    ...
    queue<pair<LL, LL>> vals;
    vals.push({L, R});
    while(vals.size() > 0){
        auto p = vals.front(); vals.pop();
        auto v = gcd(p.second, p.first);
        if(v == 1LL){
            out(p.second - p.first); return 0;
        }

        if(p.first + 1LL < p.second){
            vals.push({p.first + 1LL,  p.second});
        }
        if(p.first < p.second - 1LL){
            vals.push({p.first, p.second - 1LL});
        }        
