...
int main(){
    ...
    REP(i, 6){
        auto l = L[i];
        if(E.find(l) != E.end()){++count;}
        if(L[i] == B){bounus = true;}
    }

    if(count == 6){out(1); return 0;}
    if(count == 5 && bounus){out(2); return 0;}
    if(count == 5){out(3); return 0;}
    if(count == 4){out(4); return 0;}
    if(count == 3){out(5); return 0;}
