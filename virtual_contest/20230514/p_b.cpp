...
int main(){
    ...
    priority_queue<LL> que_even;
    priority_queue<LL> que_odd;
    REP(i, N){
        if(A[i] % 2 == 0){
            que_even.push(A[i]);
        }else{
    ...
    if(que_even.size() <= 1 && que_odd.size() <= 1){
        out(-1);
        return 0;
    }
    ...
    vector<LL> values;
    if(que_even.size() >= 2){
        auto v1 = que_even.top(); que_even.pop();
        auto v2 = que_even.top(); que_even.pop();
        values.push_back(v1 + v2);
    }

    if(que_odd.size() >= 2){
        auto v1 = que_odd.top(); que_odd.pop();
        auto v2 = que_odd.top(); que_odd.pop();
        values.push_back(v1 + v2);
    }

    LL ans = *max_element(ALL(values));
    out(ans);
    return 0;
}