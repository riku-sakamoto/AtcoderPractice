...
int main(){
    ...
    vector<vector<LL>> values(N + 1, vector<LL>(2, 0LL));

    REP(i, N){
        if(A[i] % 2 == 0){
            values[i + 1][0] = 2LL * values[i][0] + 1LL;
            values[i + 1][1] = 2LL * values[i][1];
        }else{
            values[i + 1][0] = values[i][1] + values[i][0];
            values[i + 1][1] = values[i][1] + values[i][0] + 1LL;
        }
    }
