...
int main(){
    int N, M;
    cin >> N >> M;
    vector<vector<LL>> B(N, vector<LL>(M, 0LL));
    REP(i, N){
        REP(j, M){
            cin >> B[i][j];
        }
    ...
    vector<vector<pair<LL, LL>>> C(N, vector<pair<LL, LL>>(M));
    REP(i, N){
        REP(j, M){
            C[i][j] = {(B[i][j] - 1LL) / 7LL, (B[i][j] - 1LL) % 7LL};
        }
    }

    REP(i, N){
        REP(j, M - 1){
            if(C[i][j + 1].second - C[i][j].second != 1LL){
                no(); return 0;
            }
            if(C[i][j + 1].first != C[i][j].first){
                no(); return 0;
            }
        }
    }
    ...
    REP(i, N - 1){
        REP(j, M){
            if(C[i + 1][j].second != C[i][j].second){
                no(); return 0;
            }
            if(C[i + 1][j].first - C[i][j].first != 1LL){
                no(); return 0;
            }
        }
    }
