...
int main(){
    ...
    priority_queue<pair<int, int>, vector<pair<int, int>>, decltype(comp)> math(comp);
    priority_queue<pair<int, int>, vector<pair<int, int>>, decltype(comp)> english(comp);
    priority_queue<pair<int, int>, vector<pair<int, int>>, decltype(comp)> all(comp);
    REP(i, N){
        math.push({A[i], i});
        english.push({B[i], i});
        all.push({C[i], i});
    ...
    set<int> passed;
    while(X > 0){
        auto v = math.top(); math.pop();
        passed.insert(v.second);
        --X;
    }


    while(Y > 0){
        auto v = english.top(); english.pop();
        if(passed.find(v.second) != passed.end()){continue;}
        passed.insert(v.second);
        --Y;
    }
