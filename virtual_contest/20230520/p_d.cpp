...
int main(){
    int N;
    cin >> N;
    double x0, y0, x_h, y_h;
    cin >> x0 >> y0 >> x_h >> y_h;

    double x_z = (x0 + x_h) / 2.0;
    double y_z = (y0 + y_h) / 2.0;
    double theta = M_PI / double(N / 2);

    double x = (x0 - x_z) * cos(theta) - (y0-y_z) * sin(theta) + x_z;
    double y = (x0 - x_z) * sin(theta) + (y0 - y_z) * cos(theta) + y_z;
