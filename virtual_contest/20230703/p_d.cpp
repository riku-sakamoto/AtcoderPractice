...
int main(){
    ...
    REPREV(i, N){
        ...
        for(auto v: edges[i]){
            if(existed.find(v) == existed.end()){continue;}
            if(counts.find(group.root(v)) != counts.end()){
                counts.erase(group.root(v));
            }
        }
        for(auto v: edges[i]){
            if(existed.find(v) == existed.end()){continue;}
        ...
        counts.insert(group.root(i));
        existed.insert(i);
        answers.push_back(counts.size());
    }
