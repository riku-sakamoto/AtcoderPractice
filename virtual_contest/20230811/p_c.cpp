...
int main(){
    ...
    pair<mint, mint> answer = {mint(A[0]), mint(0LL)};
    pair<mint, mint> count = {1, 0};

    FOR(i, 1, N){
        mint v0 = answer.first + answer.second;
        v0 += (count.first + count.second) * A[i];

        mint v1 = answer.first;
        v1 -= count.first * A[i];
        answer = {v0, v1};

        mint c0 = count.first + count.second;
        mint c1 = count.first;
        count = {c0, c1};
        
    }

    mint answer_v = answer.first + answer.second;
    cout << answer_v.val() << endl;
    return 0;
}