...
int main(){
    ...
    vector<vector<pair<int, int>>> counts(R, vector<pair<int, int>>(C));
    REP(i, R){
        REP(j, C){
            if(maze[i][j] == 'x'){
                counts[i][j].first = 0; continue;
            }
            int pre;
            if(i == 0){pre = 0;}
            else{
                pre = counts[i - 1][j].first;
            }

            pre++;
            counts[i][j].first = pre;
        }
    }
    ...
    REPREV(i, R){
        REPREV(j, C){
            if(maze[i][j] == 'x'){
                counts[i][j].second = 0; continue;
            }
            int pre;
            if(i == R - 1){pre = 0;}
            else{
                pre = counts[i + 1][j].second;
            }

            pre++;
            counts[i][j].second = pre;
        }
    }

    // REP(i, R){
    //     REP(j, C){
    //         cout << counts[i][j].first << "/" << counts[i][j].second << " ";
    //     }
    //     cout << endl;
    // }
    ...
    REP(i, R){
        REP(j, C){
            ...
            REP(k, K){
                ...
                //     cout << i + k << ", " << j << endl;
                // }
                if(counts[i][j - k].first < K - k){
                    ok = false; break;
                }
                if(counts[i][j - k].second < K - k){
                    ok = false; break;
                }
                if(counts[i][j + k].first < K - k){
                    ok = false; break;
