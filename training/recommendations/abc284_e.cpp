...
void dfs(int node, set<int>& route, vector<vector<int>>& graph, vector<bool>& seen_forward){
    
    for(auto to: graph[node]){
        if(route.find(to) != route.end()){continue;}
        route.insert(to);
        dfs(to, route, graph, seen_forward);
        route.erase(to);
    }
    seen_forward[node] = true;
...
int main(){
    ...
    vector<bool> seen_forward(N, false);
    set<int> route = {0};
    seen_forward[0] = true;
    dfs(0, route, graph, seen_forward);
