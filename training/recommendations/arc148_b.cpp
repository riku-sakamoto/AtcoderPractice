...
string convert(string S){
    ...
    int N = S.size();
    REPREV(i, N){
        if(S[i] == 'p'){tmpS.push_back('d');}
        else{tmpS.push_back('p');}
    }
    return tmpS;
}
...
int main(){
    ...
    string start = str_slice(0, idx, S);
    set<string> vals;
    FOR(j, idx + 1, N + 1){
        auto tmp = str_slice(idx, j, S);
        auto tmps = convert(tmp);
        auto complete = start + tmps + str_slice(j, N, S);
        vals.insert(complete);
    }
