...
int main(){
    ...
    set<tuple<int, int, pair<int, int>>> lights;
    ...
    queue<tuple<int, int, pair<int, int>>> que;

    while(!lights.empty() || !que.empty()){
        int px, py;
        pair<int, int> d;
        if(que.empty()){
            auto item = *lights.begin();
            lights.erase(item);
            que.push(item);
        }
        tie(px, py, d) = que.front();
        que.pop();

        bright[px][py] = true;
        auto n_px = px + d.first;
        auto n_py = py + d.second;

        if(n_px < 0 || n_px >= H || n_py < 0 || n_py >= W){
            continue;
        }

        if(maze[n_px][n_py] == '#'){continue;}
        if(maze[n_px][n_py] == 'o'){
            if(lights.find({n_px, n_py, d}) != lights.end()){
                lights.erase({n_px, n_py, d});
            }
            pair<int, int> rev_d = {-1 * d.first, -1 * d.second};
            if(lights.find({n_px, n_py, rev_d}) != lights.end()){
                lights.erase({n_px, n_py, rev_d});
            }
        }

        que.push({n_px, n_py, d});
