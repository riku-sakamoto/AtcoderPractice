...
void dfs(int node, int parent, vector<vector<int>>& edges, vector<int>& n_children){
    int tmp_children = 0;
    for(auto v: edges[node]){
        if(v == parent){continue;}
        dfs(v, node, edges, n_children);
        tmp_children += max({1, n_children[v]});
    }

    n_children[node] = tmp_children;
    return;
}

void dfs_distribute(int node, int parent, vector<vector<int>>& edges, vector<int>& n_children, vector<pair<int, int>>& answers){

    int l = answers[node].first;
    int r = answers[node].second;
    for(auto v: edges[node]){
        if(v == parent){continue;}

        int n_child = n_children[v];
        int tmpr = max({n_child - 1, 0});
        answers[v] = {l, l + tmpr};
        l += (tmpr + 1);

        dfs_distribute(v, node, edges, n_children, answers);
        // cout << "l: " << l << ", tmpr: " << tmpr << endl;
    }
}
...
int main(){
    ...
    vector<int> n_children(N, 0);
    dfs(0, -1, edges, n_children);

    // cout << n_children[0] << endl;
    vector<pair<int, int>> answers(N);
    int tmp = max({n_children[0] - 1, 0});
    answers[0] = {1, 1 + tmp};
    dfs_distribute(0, -1, edges, n_children, answers);
    REP(i, N){
