...
int main(){
    ...
    vector<pair<LL, LL>> dates(D);
    REP(i, D){
        LL l, r; cin >> l >> r;
        dates[i] = {l, r};
    }
    vector<pair<LL, LL>> translates(K);
    REP(i, K){
        LL s, t; cin >> s >> t;
        translates[i] = {s, t};
    }
    ...
    REP(i, K){
        auto p = translates[i];
        pair<LL, LL> pos = {p.first, p.first};
        REP(j, D){
            auto d = dates[j];
            if(d.second < pos.first || d.first > pos.second){continue;}
            chmin(pos.first, d.first);
            chmax(pos.second, d.second);
            if(pos.first<= p.second && p.second <= pos.second){
