...
int main(){
    ...
    int N = S.size();
    map<char, set<char>> changes_out;
    map<char, set<char>> changes_in;
    REP(i, N){
        changes_in[T[i]].insert(S[i]);
        changes_out[S[i]].insert(T[i]);
    }

    for(auto p: changes_in){
        if(p.second.size() != 1){
            no();
            return 0;
        }
    }
    for(auto p: changes_out){
        if(p.second.size() != 1){
            no(); return 0;
        }
    }
