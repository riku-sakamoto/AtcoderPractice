...
int main(){
    ...
    auto vec = rle_compress(S);
    int k = vec.size();
    vector<int> sums(k + 1, 0);
    REP(i, k){
        sums[i + 1] = sums[i] + vec[i].second;
    }
    ...
    REP(i, m){
        int j = min({i + K - 1, m - 1});
        int tmp_i = max({zeros[i].first - 1, 0});
        int tmp_j = min({zeros[j].first + 2, k});
        // cout << tmp_i << ", " << tmp_j << endl;
        auto tmp = sums[tmp_j] - sums[tmp_i];
        chmax(ans, tmp);
    }
    out(ans);
    return 0;
