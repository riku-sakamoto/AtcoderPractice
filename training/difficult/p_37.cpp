...
int main(){
    ...
    vector<vector<pair<int, int>>> pos(H);
    set<int> rows;
    int tmp_num = 0;
    REP(i, H){
        REP(j, W){
            ...
            if(cake[i][j] == '#'){
                ++tmp_num;
                pos[i].push_back({j, tmp_num});
                rows.insert(i);
            }
        }
    }
    vector<vector<int>> answers(H, vector<int>(W, 0));
    REP(i, H){
        if(pos[i].size() == 0){continue;}
        int idx = 0;
        auto num = pos[i][idx].second;
        REP(j, W){
            answers[i][j] = num;            
            if(cake[i][j] == '#'){
                ++idx;
                if(idx < pos[i].size()){
                    num = pos[i][idx].second;
                }
            }
        }
    }
    ...
    REP(i, H){
        REP(j, W){
            auto itr = rows.lower_bound(i);
            if(itr == rows.end()){
                --itr;
            }
            if(answers[i][j] == 0){
                answers[i][j] = answers[*itr][j];
            }
        }
    }
