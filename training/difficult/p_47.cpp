...
int main(){
    int K; cin >> K;
    set<LL> values;
    priority_queue<LL, vector<LL>, greater<LL>> tmp_values;
    REP(i, 9){tmp_values.push(i + 1); values.insert(i + 1);}

    while (values.size() < K)
    {
        priority_queue<LL, vector<LL>, greater<LL>> next_values;
        while(!tmp_values.empty()){
            auto v = tmp_values.top(); tmp_values.pop();
            auto c = v % 10;

            v *= 10;
            if(c == 9){
                next_values.push(v + c);
                next_values.push(v + c - 1);
                values.insert(v + c);
                values.insert(v + c - 1);
            }else if(c == 0){
                next_values.push(v + c);
