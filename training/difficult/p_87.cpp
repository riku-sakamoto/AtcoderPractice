...
int main(){
    ...
    UnionFind friend_groups(N);
    map<int, set<int>> friends;
    REP(i, M){
        int a, b; cin >> a >> b;
        --a; --b;
        friend_groups.unite(a, b);
        friends[a].insert(b);
        friends[b].insert(a);
    }
    map<int, int> counts;
    REP(i, N){
        counts[friend_groups.root(i)] += 1;
    }
    ...
    REP(i, N){
        auto v = counts[friend_groups.root(i)] - friends[i].size() - 1;
        v -= blocks[i].size();
        answers[i] = v;
    }
