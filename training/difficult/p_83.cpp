...
int main(){
    ...
    vector<vector<bool>> blacks(H, vector<bool>(W, false));
    ...
    REP(i, H){
        REP(j, W){
            ...
            for(auto p: dirs){
                ...
                if(blacks[next_i][next_j]){continue;}
                coords.insert({next_i, next_j});
            }
        }
    }
    ...
    while(true){
        ...
        ++count;
        set<pair<int, int>> tmp_coords;
        for(auto coord: coords){
            blacks[coord.first][coord.second] = true;
        }
        for(auto coord: coords){
            for(auto p: dirs){
                auto next_i = coord.first + p.first;
                auto next_j = coord.second + p.second;
                if(next_i < 0 || next_i >= H || next_j < 0 || next_j >= W){
                    continue;
                }
                if(blacks[next_i][next_j]){continue;}
                tmp_coords.insert({next_i, next_j});
            }
        }
        coords = tmp_coords;
    }
