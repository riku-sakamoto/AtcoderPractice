...
int main(){
    ...
    vector<LL> count_B(N, 0);
    REP(i, N){
        auto b = B[i];
        int idx = larger_than(C, b);
        count_B[i] = N - idx;
    }
    vector<LL> sum_count_B(N, 0);
    REPREV(i, N){
        if(i == N - 1){sum_count_B[i] = count_B[N - 1];}
        else{
            sum_count_B[i] = sum_count_B[i + 1] + count_B[i];
        }
    }
