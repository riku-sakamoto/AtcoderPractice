...
int main(){
    ...
    REP(i, X){pque.push(p[i]);}
    REP(i, Y){qque.push(q[i]);}

    REP(i, C){
        auto rval = r[i];
        auto pval = pque.top();
        auto qval = qque.top();
        if(pval >= rval && qval >= rval){break;}

        if(pval >= rval && qval < rval){
            qque.pop(); qque.push(rval); continue;
        }

        if(pval < rval && qval >= rval){
            pque.pop(); pque.push(rval); continue;
        }
    ...
    LL ans = 0;
    while(!pque.empty()){
        auto v = pque.top(); pque.pop();
        ans += v;
    }
    while(!qque.empty()){
        auto v = qque.top(); qque.pop();
        ans += v;
    }
    out(ans);
    return 0;
