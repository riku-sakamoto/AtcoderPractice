...
int main(){
    ...
    int K;
    cin >> K;
    map<int, set<int>> not_matched;
    REP(i, K){
        int x, y;
        cin >> x >> y;
        --x; --y;
        int g_x = group.root(x);
        int g_y = group.root(y);
        not_matched[g_x].insert(g_y);
        not_matched[g_y].insert(g_x);
    }
    ...
    REP(i, Q){
        ...
        int g_p = group.root(p);
        int g_q = group.root(q);

        if(not_matched.find(g_p) != not_matched.end()){
            if(not_matched[g_p].find(g_q) != not_matched[g_p].end()){
                que.push("No");
                continue;
            }
        }
        if(not_matched.find(g_q) != not_matched.end()){
            if(not_matched[g_q].find(g_p) != not_matched[g_q].end()){
                que.push("No");
                continue;
            }
        }
