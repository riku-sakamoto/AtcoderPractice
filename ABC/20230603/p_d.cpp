...
int main(){
    ...
    REP(i, N){
        ...
        int idx_c = lower_than(cols, _col);
        int idx_r = lower_than(rows, _row);

        counts[{idx_c, idx_r}] += 1;
    }

    int max_val = 0;
    int min_val = inf;
    for(auto v: counts){
        chmin(min_val, v.second);
        chmax(max_val, v.second);
    }
    LL n = counts.size();
    if(n < LL((LL)A + 1LL) * LL((LL)B + 1LL)){
        min_val = 0;
    }
