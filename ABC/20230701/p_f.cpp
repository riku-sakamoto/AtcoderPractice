...
int main(){
    ...
    // D, L
    vector<pair<LL, LL>> coupons(M, {0LL, 0LL});
    REP(i, M){
        cin >> coupons[i].second;
    }
    REP(i, M){
        cin >> coupons[i].first;
    }
    ...
    priority_queue<pair<LL, LL>, vector<pair<LL, LL>>, decltype(comp)> que(comp);
    REP(i, M){
        que.push(coupons[i]);
    }

    LL ans = 0LL;
    vector<LL> applys(N, 0LL);
    REP(_, M){
        auto p = que.top(); que.pop();
        auto target = P.lower_bound(p.second);
        if(target == P.end()){continue;}
        ans += *target - p.first;
        P.erase(target);
    }
