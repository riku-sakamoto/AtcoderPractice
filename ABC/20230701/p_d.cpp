...
bool dfs(vector<vector<char>>& maze, int i, int j, int next_c, vector<vector<bool>>& seen){
    auto c = snuke[next_c % 5];
    if(maze[i][j] != c){return false;}
    ...
    seen[i][j] = true;
    for(auto dir: dirs){
        auto next_i = i + dir.first;
        auto next_j = j + dir.second;
        if(next_i < 0 || next_i >= H || next_j < 0 || next_j >= W){continue;}
        if(seen[next_i][next_j]){continue;}
        auto ok = dfs(maze, next_i, next_j, next_c + 1, seen);
        if(ok){return true;}
    }
    return false;
}
