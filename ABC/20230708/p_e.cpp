...
int main(){
    ...
    REP(i, N - 1){
        int p; cin >> p; --p;
        childrens[p].push_back(i + 1);
    }

    map<int, int> insurance;
    REP(i, N){insurance[i] = -1;}
    REP(i, M){
        int x, y; cin >> x >> y; --x;
        chmax(insurance[x], y);
    }
    ...
    que.push({0, insurance[0]});
    while(!que.empty()){
        auto p = que.front(); que.pop();

        for(auto v: childrens[p.first]){
            auto ins = max({p.second - 1, insurance[v]});
            vals[v] = ins;
            que.push({v, ins});
        }
    }
