...
int solve(int K, int start, vector<vector<int>>& edges){
...
int main(){
    ...
    vector<vector<int>> edgesA(N1);
    vector<vector<int>> edgesB(N1 + N2);

    REP(i, M){
        int a, b; cin >> a >> b;
        if(a <= N1){
            --a; --b;
            edgesA[a].push_back(b);
            edgesA[b].push_back(a);
        }
        if(a > N1){
            --a; --b;
            edgesB[a].push_back(b);
            edgesB[b].push_back(a);
        }
    }

    int ans1 = solve(N1, 0, edgesA);
    int ans2 = solve(N1 + N2, N1 + N2 - 1, edgesB);
