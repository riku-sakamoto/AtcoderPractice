...
int main(){
    int N; cin >> N;
    vector<tuple<LL, LL, int>> elections(N);
    int sum_z = 0;
    REP(i, N){
        LL x, y; int z; cin >> x >> y >> z;
        elections[i] = {x, y, z};
        sum_z += z;
    }
    ...
    int t_val = 0;
    int a_val = 0;
    vector<pair<LL, int>> cost_gain;
    REP(i, N){
        LL x, y; int z;
        tie(x, y, z) = elections[i];
        if(y > x){a_val += z;}
        else{t_val += z;}

        if(y > x){
            LL cost = (y - x + 1LL) / 2LL;
            cost_gain.push_back({cost, z});
        }
    }
    ...
    REP(i, n){
        ...
        int gain = p.second;
        REP(j, sum_z + 1){
            auto min_v = max({j - gain, 0});
            if(dp[i][min_v] == INF){continue;}

            auto new_cost = cost + dp[i][min_v];
            auto new_v = min({dp[i][j], new_cost});
            chmin(dp[i + 1][j], new_v);
        }
    }

    auto n_cost = (a_val - t_val + 1LL) / 2LL;
    LL ans = INF;
    FOR(j, n_cost, sum_z + 1){
        chmin(ans, dp[n][j]);
    }
