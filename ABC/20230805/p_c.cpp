...
LL calc_num(LL val, vector<LL>& A){
    LL n = 0LL; LL m = 0LL;
    REP(i, N){
        if(A[i] <= val - 1LL){
            n += val - 1LL - A[i];
...
LL user_binary_search(LL min_v, LL max_v, vector<LL>& A){
    if(max_v - min_v <= 1){
        return min_v;
    }
    auto v = (min_v + max_v) / 2LL;
    auto flag = calc(v, A);
    if(flag){
        return user_binary_search(v, max_v, A);
    }else{
        return user_binary_search(min_v, v, A);
    }
}
...
int main(){
    ...
    LL min_v = 0LL;
    LL max_v = 1000000000LL + 1LL;
    auto v = user_binary_search(min_v, max_v, A);

    
    auto ans = calc_num(v, A);
    auto ans2 = calc_num(v + 1LL, A);
    if(ans > ans2){
        out(ans2);
    }else{
        out(ans);
