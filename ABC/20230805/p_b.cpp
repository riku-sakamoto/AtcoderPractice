...
void dfs(int node, vector<vector<int>>& strongers, vector<bool>& seen){
    
    seen[node] = true;
    if(strongers[node].size() == 0){
        saikyo.insert(node);
        return;
    }

    for(auto v: strongers[node]){
        if(seen[v]){continue;}
        dfs(v, strongers, seen);
    }
...
int main(){
    int N, M; cin >> N >> M;
    vector<vector<int>> strongers(N);
    vector<bool> seen(N, false);

    REP(i, M){
        int a, b; cin >> a >> b; --a; --b;
        strongers[b].push_back(a);
    }

    
    REP(i, N){
        if(seen[i]){continue;}
        dfs(i, strongers, seen);
    }
