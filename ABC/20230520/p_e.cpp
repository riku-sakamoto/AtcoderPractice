...
int main(){
    ...
    for(auto item: querys){
        if(item.second == -1){
            for(auto v: groups[item.first]){
                groups[v].erase(item.first);
                if(groups[v].size() == 0){
                    ans += 1;
                }
            }
            if(groups[item.first].size() != 0){
                ans += 1;
            }
            groups[item.first].clear();
            answers.push(ans);
            continue;
        }
        ...
        groups[item.first].insert(item.second);
        if(groups[item.second].size() == 0){
            --ans;
        }
    ...
    while(!answers.empty()){
        auto v = answers.front(); answers.pop();
        out(v);
    }
