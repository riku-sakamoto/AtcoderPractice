...
bool matched(vector<vector<char>>& A, vector<pair<int, int>>& trans, int i, int j){
    REP(k, 5){
        auto p = trans[k];
        if(A[i + p.first][j + p.second] != target[k]){
            return false;
        }
    }
    return true;
...
void show(vector<pair<int, int>>& vec, int i, int j){
    vector<pair<int, int>> indexes;
    for(auto p: vec){
        indexes.push_back({i + p.first, j + p.second});
    }
...
int main(){
    ...
    REP(i, H){
        REP(j, W){
            if(j + 4 < W){
                vector<pair<int, int>> vec = {{0, 0}, {0, 1}, {0, 2}, {0, 3}, {0, 4}};

                bool ok = matched(A, vec, i, j);
                if(ok){
                    show(vec, i, j);
                    return 0;
                }
            ...
            if(i + 4 < H){
                vector<pair<int, int>> vec = {{0, 0}, {1, 0}, {2, 0}, {3, 0}, {4, 0}};
                bool ok = matched(A, vec, i, j);
                if(ok){
                    show(vec, i, j);
                    return 0;
            ...
            if(i + 4 < H && j + 4 < W){
                vector<pair<int, int>> vec = {{0, 0}, {1, 1}, {2, 2}, {3, 3}, {4, 4}};
                bool ok = matched(A, vec, i, j);
                if(ok){
                    show(vec, i, j);
                    return 0;
