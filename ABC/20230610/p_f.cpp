...
void dfs(vector<vector<int>>& graph, vector<bool>& seen, int next_i, vector<int>& pre){
    if(!first){
        next_v = next_i + 1;
        cout << next_v << endl;
        cout << flush;
    ...
    int k; cin >> k;
    vector<int> nodes(k);
    REP(i, k){cin >> nodes[i]; --nodes[i];}

    graph[next_i] = nodes;
    seen[next_i] = true;
    // cout << "vectors" << endl;
    // out_vector(pre);
    for(auto v: graph[next_i]){
        if(seen[v]){continue;}
        pre[v] = next_i;
        dfs(graph, seen, v, pre);
        break;
    }
    int v = pre[next_i];
    // cout << "v: " << v << endl;
