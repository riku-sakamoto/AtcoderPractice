...
void dfs(int num, vector<set<int>>& teams, vector<vector<int>>& non_matched){
    if(num == N){
        if(teams.size() == T){
            ++ans;
        }
    ...
    // チーム数がT未満なら新しいチームとして登録してみる
    if(teams.size() < T){
        teams.push_back({num});
        dfs(num + 1, teams, non_matched);
        teams.pop_back();
    }

    for(auto& t: teams){
        bool ok = true;
        for(auto anti: non_matched[num]){
            if(t.find(anti) != t.end()){
                ok = false; break;
            }
        }
        if(!ok){continue;}
...
int main(){
    cin >> N >> T >> M;
    vector<vector<int>> non_matched(N);
    REP(i, M){
        int a, b; cin >> a >> b;
        --a; --b;
        non_matched[a].push_back(b);
        non_matched[b].push_back(a);
    }

    vector<set<int>> teams;
    dfs(0, teams, non_matched);
