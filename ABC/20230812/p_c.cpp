...
int main(){
    ...
    map<int, vector<char>> color2chars;
    map<int, int> index2colorindex;
    REP(i, N){
        index2colorindex[i] = color2chars[colors[i]].size();
        color2chars[colors[i]].push_back(S[i]);
    }

    // out_vector(color2chars[2]);
    vector<char> answers;
    REP(i, N){
        auto c = colors[i];
        auto c_idx = index2colorindex[i];
        auto n = color2chars[c].size();
        auto idx = (c_idx + n - 1) % n;
        // cout << c << ", " << c_idx << ", " << n <<  ", " << idx << endl;
        auto a = color2chars[c][idx];
        answers.push_back(a);
    }
