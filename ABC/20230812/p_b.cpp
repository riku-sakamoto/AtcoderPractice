...
int main(){
    int N; cin >> N;
    map<int, vector<int>> point2person;
    map<int, int> person2count;
    REP(i, N){
        int c; cin >> c;
        REP(j, c){
            int a; cin >> a;
            person2count[i] += 1;
            point2person[a].push_back(i);
        }
    }
    int X; cin >> X;

    auto persons = point2person[X];

    int min_val = inf;
    for(auto p: persons){
        chmin(min_val, person2count[p]);
    }

    vector<int> answers;
    for(auto p: persons){
        if(person2count[p] == min_val){
            answers.push_back(p + 1);
        }
    }
    sort(ALL(answers));
