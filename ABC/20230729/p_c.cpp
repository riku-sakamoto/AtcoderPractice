...
LL solve(LL x, vector<LL>& A, vector<LL>& B){
    auto idx_a = lower_equal(A, x);
    auto idx_b = larger_equal(B, x);
...
LL b_search(LL x1, LL x2, vector<LL>& A, vector<LL>& B){
    if(x2 - x1 <= 1LL){return x2;}
    auto x = (x1 + x2) / 2LL;

    if(solve(x, A, B)){
        return b_search(x1, x, A, B);
    }else{
        return b_search(x, x2, A, B);
    }
...
int main(){
    ...
    auto x = b_search(0LL, 1000000001LL, A, B);
    out(x);
