...
string solve(){
    ...
    REP(i, n - 1){
        auto v = values[i];
        if(v == 0LL){return "NO";}
        if(v > n){left.push_back(v); continue;}
        if(found[v - 1] != false){left.push_back(v); continue;}
        found[v - 1] = true;
    }

    vector<LL> no_seen;
    REP(i, n){
        if(found[i] == false){no_seen.push_back(i + 1);}
    }

    if(left.size() == 0 && no_seen.size() == 1){
        return "YES";
    }

    if(no_seen.size() != 2){return "NO";}
    if(left.size() != 1){return "NO";}

    auto v = no_seen[0] + no_seen[1];
    if(left[0] != v){return "NO";}
