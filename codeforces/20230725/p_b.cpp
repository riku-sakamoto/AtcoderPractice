...
string solve(){
    ...
    priority_queue<int, vector<int>, greater<int>> que_even;
    priority_queue<int, vector<int>, greater<int>> que_odd;
    REP(i, N){
        if(A[i] % 2LL == 0){
            que_even.push(A[i]);
        }else{
    ...
    REP(i, N){
        if(A[i] % 2LL == 0){
            auto v = que_even.top(); que_even.pop();
            B.push_back(v);
        }else{
            auto v = que_odd.top(); que_odd.pop();
            B.push_back(v);
        }
    }
