...
LL user_binary_search(LL l, LL r, LL target, vector<LL>& A){
    if(r - l <= 1LL){return l;}
    int N = A.size();
    auto m = (l + r) / 2LL;
    LL area = 0LL;
    ...
    if(area < target){
        return user_binary_search(m, r, target, A);
    }else{
        return user_binary_search(l, m, target, A);
    }
}
...
LL solve(){
    ...
    auto v = user_binary_search(0LL, _max, c, A);
    return v;
}
...
int main(){
    ...
    vector<LL> answers;
    REP(_, T){
        auto v = solve(); answers.push_back(v);
    }
