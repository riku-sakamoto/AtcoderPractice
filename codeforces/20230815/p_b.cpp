...
pair<LL, int> solve(){
    ...
    LL cookies = 1LL;
    LL before = 1LL;
    map<LL, LL> cookies_vals;
    cookies_vals[1LL] = 1LL;
    
    REP(i, m){
        auto s = sellers[i];
        if(s == 1LL){continue;}
        auto k = (s - before - 1LL) / d;
        cookies += k;
        cookies += 1LL;
        before = s;
        cookies_vals[s] = cookies;

        if(i == m - 1){
            k = (n - s) / d;
            cookies += k;
            cookies_vals[n] = cookies;
        }
    }

    // for(auto p: cookies_vals){
    ...
    REP(i, m){
        LL before = 1LL;
        LL after = n;
        if(i != 0){before = sellers[i - 1];}
        if(i != m - 1){after = sellers[i + 1];}

        auto l = after - before;
        auto tmp_l = cookies_vals[before];
        auto tmp_r = cookies_vals[n] - cookies_vals[after] + 1LL;
        if(i == m - 1){
            tmp_r -= 1LL; l += 1LL;
        }
        auto tmp_m = max({l - 1LL, 0LL}) / d;

        // cout << tmp_l << ", " << tmp_r << ", " << tmp_m << endl;
        // cout << l << endl;
        auto v = tmp_l + tmp_r + tmp_m;
        // cout << i << ": " << v << endl;
        if(ans == v){
            // cout << i << endl;
            stand++; continue;
