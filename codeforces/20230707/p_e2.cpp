...
bool binary_search(LL left, LL right, LL N){
    if(right - left <= 1LL){
        if(calc(left) == N){return true;}
        else{return false;}
    }
    
    auto mid = (left + right) / 2;
    if(calc(mid) <= N){
        return binary_search(mid, right, N);
    }else{
...
string solve(set<LL>& vals){
    ...
    if(calc(left) == N){return "YES";}
    if(calc(right) == N){return "YES";}
...
int main(){
    set<LL> vals;
    for(LL m = 2LL; 1LL + m + m * m + m * m * m < INF; ++m){
        auto k = 1LL + m + m * m + m * m * m;
        auto last = m * m * m;
        REP(i, 64){
            vals.insert(k);
            last *= m;
